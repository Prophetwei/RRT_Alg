...
module RRT_TOP (
    ...
    logic        p0_acc, p1_acc, p2_acc, p3_acc;
    logic        p0_rdy, p1_rdy, p2_rdy, p3_rdy;
    logic [3:0]  p0_rgx,p0_rgy,p0_rgz,p0_rslot, p1_rgx,p1_rgy,p1_rgz,p1_rslot;
    logic [3:0]  p2_rgx,p2_rgy,p2_rgz,p2_rslot, p3_rgx,p3_rgy,p3_rgz,p3_rslot;
    ...
    logic        s_pe_b_req, g_pe_b_req;
    logic [3:0]  s_pe_b_gx, s_pe_b_gy, s_pe_b_gz, s_pe_b_slot;
    logic [3:0]  g_pe_b_gx, g_pe_b_gy, g_pe_b_gz, g_pe_b_slot;
    ...
    // -------- PE <-> MAPCONT --------
    logic        m0_en,m1_en,m2_en,m3_en, m0_acc,m1_acc,m2_acc,m3_acc;
    logic        m0_rdy,m1_rdy,m2_rdy,m3_rdy, m0_ob,m1_ob,m2_ob,m3_ob;
    logic [23:0] m0_node,m1_node,m2_node,m3_node;
    logic        map_obstacle_seen;
    ...
    PE #(.SEED(8'd255), .STEP_DOWN(-5'sd1)) u_PE0 (
        ...
        .read_rsp_gx(p0_rgx), .read_rsp_gy(p0_rgy), .read_rsp_gz(p0_rgz), .read_rsp_slot(p0_rslot),
        .node_valid(p0_nv), .node_out(p0_node), .parent_out(p0_par), .node_ack(p0_ack),
        .read_map_en(m0_en), .map_node(m0_node), .map_accept(m0_acc), .map_ready(m0_rdy), .obstacle(m0_ob),
        .wide_nn(map_obstacle_seen)
    );
    PE #(.SEED(8'd255), .STEP_DOWN(-5'sd1)) u_PE1 (
        ...
        .read_rsp_gx(p1_rgx), .read_rsp_gy(p1_rgy), .read_rsp_gz(p1_rgz), .read_rsp_slot(p1_rslot),
        .node_valid(p1_nv), .node_out(p1_node), .parent_out(p1_par), .node_ack(p1_ack),
        .read_map_en(m1_en), .map_node(m1_node), .map_accept(m1_acc), .map_ready(m1_rdy), .obstacle(m1_ob),
        .wide_nn(map_obstacle_seen)
    );
    PE #(.SEED(8'd0), .STEP_DOWN(5'sd1)) u_PE2 (
        ...
        .read_rsp_gx(p2_rgx), .read_rsp_gy(p2_rgy), .read_rsp_gz(p2_rgz), .read_rsp_slot(p2_rslot),
        .node_valid(p2_nv), .node_out(p2_node), .parent_out(p2_par), .node_ack(p2_ack),
        .read_map_en(m2_en), .map_node(m2_node), .map_accept(m2_acc), .map_ready(m2_rdy), .obstacle(m2_ob),
        .wide_nn(map_obstacle_seen)
    );
    PE #(.SEED(8'd0), .STEP_DOWN(5'sd1)) u_PE3 (
        ...
        .read_rsp_gx(p3_rgx), .read_rsp_gy(p3_rgy), .read_rsp_gz(p3_rgz), .read_rsp_slot(p3_rslot),
        .node_valid(p3_nv), .node_out(p3_node), .parent_out(p3_par), .node_ack(p3_ack),
        .read_map_en(m3_en), .map_node(m3_node), .map_accept(m3_acc), .map_ready(m3_rdy), .obstacle(m3_ob),
        .wide_nn(map_obstacle_seen)
    );
    ...
    TREECONT u_TC_G (
        ...
        .b_available(!(cc_rd_en && (cc_rd_sel == 1'b1))),
        .b_read_en(g_pe_b_req), .b_gx(g_pe_b_gx), .b_gy(g_pe_b_gy), .b_gz(g_pe_b_gz), .b_slot(g_pe_b_slot),
    ...
    MAPCONT u_MC (
        .clk(clk), .rst_n(rst_n),
        .pe0_read_en(m0_en), .pe0_read_node(m0_node), .pe0_read_accept(m0_acc), .pe0_read_ready(m0_rdy), .pe0_read_obstacle_out(m0_ob),
        .pe1_read_en(m1_en), .pe1_read_node(m1_node), .pe1_read_accept(m1_acc), .pe1_read_ready(m1_rdy), .pe1_read_obstacle_out(m1_ob),
        .pe2_read_en(m2_en), .pe2_read_node(m2_node), .pe2_read_accept(m2_acc), .pe2_read_ready(m2_rdy), .pe2_read_obstacle_out(m2_ob),
        .pe3_read_en(m3_en), .pe3_read_node(m3_node), .pe3_read_accept(m3_acc), .pe3_read_ready(m3_rdy), .pe3_read_obstacle_out(m3_ob),
        .read_map_en(map_read_en), .read_x(map_read_x), .read_y(map_read_y), .read_z(map_read_z),
        .read_obstacle_in(map_obstacle), .obstacle_seen(map_obstacle_seen)
    );
    ...
    CC u_CC (
        ...
        .obstacle_seen(map_obstacle_seen),
    ...
    CCREADCONT u_CCR (
        ...
        .cc_write_gx(cc_wr_gx), .cc_write_gy(cc_wr_gy), .cc_write_gz(cc_wr_gz),
        .cc_write_node(cc_wr_node), .cc_write_parent(cc_wr_par),
        .s_pe_read_en(s_pe_b_req), .s_pe_gx(s_pe_b_gx), .s_pe_gy(s_pe_b_gy), .s_pe_gz(s_pe_b_gz), .s_pe_slot(s_pe_b_slot),
        .g_pe_read_en(g_pe_b_req), .g_pe_gx(g_pe_b_gx), .g_pe_gy(g_pe_b_gy), .g_pe_gz(g_pe_b_gz), .g_pe_slot(g_pe_b_slot),
