...
//  back to the CC.  CC has priority on the selected tree's B port; an idle
//  B port is lent to that tree's TREECONT for a second NN read.
//======================================================================
module CCREADCONT (
    // ---- from CC : read ----
    ...
    // ---- opportunistic PE reads for each tree's port B ----
    input  logic        s_pe_read_en,
    input  logic [3:0]  s_pe_gx, s_pe_gy, s_pe_gz, s_pe_slot,
    input  logic        g_pe_read_en,
    input  logic [3:0]  g_pe_gx, g_pe_gy, g_pe_gz, g_pe_slot,
    ...
    logic cc_reads_s, cc_reads_g;
    assign cc_reads_s = cc_read_en && (cc_read_sel == 1'b0);
    assign cc_reads_g = cc_read_en && (cc_read_sel == 1'b1);

    assign s_b_read_en = cc_reads_s || s_pe_read_en;
    assign g_b_read_en = cc_reads_g || g_pe_read_en;
    assign s_b_gx   = cc_reads_s ? cc_read_gx   : s_pe_gx;
    assign s_b_gy   = cc_reads_s ? cc_read_gy   : s_pe_gy;
    assign s_b_gz   = cc_reads_s ? cc_read_gz   : s_pe_gz;
    assign s_b_slot = cc_reads_s ? cc_read_slot : s_pe_slot;
    assign g_b_gx   = cc_reads_g ? cc_read_gx   : g_pe_gx;
    assign g_b_gy   = cc_reads_g ? cc_read_gy   : g_pe_gy;
    assign g_b_gz   = cc_reads_g ? cc_read_gz   : g_pe_gz;
    assign g_b_slot = cc_reads_g ? cc_read_slot : g_pe_slot;
