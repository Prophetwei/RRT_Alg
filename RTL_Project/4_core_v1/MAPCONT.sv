...
//  wraparound.  The PE receiving the current response is skipped because its
//  level request drops only on that response edge.
//
...
module MAPCONT (
    ...
    // Mask the current response owner, then use one rotating-priority decoder.
    always_comb begin
