...
//  (cell = voxel[7:4]).  Each cell holds up to 15 nodes.  A node record is
...
//  16'hFFFF.  Slot 15 is reserved, so that value cannot alias a node address.
//
...
module TREEMEM #(
    ...
    output logic [15:0] tree_size          // total node count (0..61440)
);
    localparam [3:0] RGX = ROOT_X[7:4];
    localparam [3:0] RGY = ROOT_Y[7:4];
    localparam [3:0] RGZ = ROOT_Z[7:4];

    logic [39:0] mem       [0:15][0:15][0:15][0:14];  // {node[23:0],parent[15:0]}
    logic [3:0]  grid_size [0:15][0:15][0:15];         // 0..15 occupancy per cell
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        else if (write_en && (grid_size[write_gx][write_gy][write_gz] < 4'd15)) begin
