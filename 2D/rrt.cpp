...
Node get_random_point(const Node& target, int bias_threshold) {
    ...
    return {static_cast<int>(dis(gen)), static_cast<int>(dis(gen)), -1};
}
    ...
    for (int i = 0; i < (int)other_tree.size(); i++) {
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                if (last_node.x + dx == other_tree[i].x && last_node.y + dy == other_tree[i].y) {
                    done_flag = true;
                    if (is_start_tree) {
...
void save_all_nodes() {
    ...
    for (const auto& node : start_tree) {
        s_file << node.x << " " << node.y << " " << node.parent_index << "\n";
    }
    s_file.close();

    std::ofstream g_file("tree/goal_nodes");
    for (const auto& node : goal_tree) {
        g_file << node.x << " " << node.y << " " << node.parent_index << "\n";
    }
    g_file.close();
...
int main() {
    ...
    // 2. Initialize Trees
    start_node = {2, 2, -1};
    goal_node = {MAP_SIZE - 5, MAP_SIZE - 5, -1};
